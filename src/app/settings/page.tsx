...
import { 
  ...
  Loader2, Save, LogOut, Moon, Sun, Volume2, Globe, Heart, Trash2, AlertTriangle 
} from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const handleClearData = () => {
    ...
    Object.keys(localStorage).forEach(key => {
      if (key.includes(user.id) || key.startsWith('kd_')) {
    ...
    return (
      <div className="min-h-screen bg-background pt-24 pb-20 flex items-center justify-center px-6">
        <div className="text-center space-y-4 max-w-sm">
          <div className="w-16 h-16 rounded-2xl bg-accent/5 border border-accent/15 flex items-center justify-center mx-auto text-accent">
            <Shield className="w-8 h-8" />
          </div>
          <h2 className="text-xl font-bold text-foreground">Access Settings</h2>
      ...
      <div className="max-w-[1000px] mx-auto px-6 lg:px-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border">
          ...
          <button
            ...
            disabled={isSaving}
            className="flex items-center justify-center gap-2 bg-accent hover:bg-accent-dark disabled:opacity-50 text-white font-bold text-[12px] px-5 py-2.5 rounded-xl shadow-[0_4px_20px_rgba(249,115,22,0.25)] transition-all"
          >
            {isSaving ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Main Content Area */}
          <div className="md:col-span-3 min-h-[400px] bg-card/40 border border-border rounded-[24px] p-6 lg:p-8">
            <AnimatePresence mode="wait">
              {activeTab === 'blueprint' && (
                ...
                >
                  <div className="space-y-1">
                    <h3 className="text-[15px] font-black text-foreground">Personal Discover Blueprint</h3>
                    <p className="text-xs text-muted-foreground">Select preferred options to fine-tune recommendation results.</p>
                  </div>

                  {/* Genres Selection */}
                  <div className="space-y-3">
                    <h4 className="text-[11px] font-bold text-muted-foreground/70 uppercase tracking-widest flex items-center gap-1.5">
                      <Heart className="w-3.5 h-3.5 text-accent" /> Favorite Genres
                    </h4>
                        ...
                        return (
                          <button
                            ...
                            className={`px-3.5 py-2 rounded-xl text-[11px] font-bold border transition-colors ${
                              active ? 'bg-accent/10 border-accent text-foreground' : 'bg-card/30 border-border text-muted-foreground hover:text-foreground'
                            }`}
                          >
                            {genre.label}
                          </button>
                  ...
                  <div className="space-y-3">
                    <h4 className="text-[11px] font-bold text-muted-foreground/70 uppercase tracking-widest flex items-center gap-1.5">
                      <Globe className="w-3.5 h-3.5 text-accent" /> Watching Languages
                    </h4>
                    <div className="flex flex-wrap gap-1.5">
                      {LANGUAGES.map(lang => {
                  ...
                  <div className="space-y-3">
                    <h4 className="text-[11px] font-bold text-muted-foreground/70 uppercase tracking-widest flex items-center gap-1.5">
                      <Tv className="w-3.5 h-3.5 text-accent" /> Connected OTT Networks
                    </h4>
              ...
              {activeTab === 'preferences' && (
                ...
                >
                  <div className="space-y-1">
                    <h3 className="text-[15px] font-black text-foreground">App Experience & Playback</h3>
                    <p className="text-xs text-muted-foreground">Adjust playback specifications and parental controls.</p>
                  </div>

                  {/* Parental Controls Toggle */}
                  <div className="flex items-center justify-between p-4 bg-card/30 border border-border rounded-2xl">
                    <div>
                      <h4 className="text-xs font-bold text-foreground">Parental Lock (R-Rated Filter)</h4>
                    ...
                    <button
                      ...
                      aria-pressed={parentalControls}
                      aria-label="Toggle parental controls"
                      className={`w-10 h-6 rounded-full p-1 transition-colors ${parentalControls ? 'bg-accent' : 'bg-muted/20'}`}
                    >
                  ...
                  <div className="flex items-center justify-between p-4 bg-card/30 border border-border rounded-2xl">
                    <div>
                      <h4 className="text-xs font-bold text-foreground">Auto-Play Trailers</h4>
                    ...
                    <button
                      ...
                      className={`w-10 h-6 rounded-full p-1 transition-colors ${autoPlayTrailers ? 'bg-accent' : 'bg-muted/20'}`}
                    >
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xs font-bold text-muted-foreground">Default Stream Quality Badge</h4>
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xs font-bold text-muted-foreground">App Interface Theme</h4>
                    <div className="flex gap-2">
                      <button
              ...
              {activeTab === 'notifications' && (
                ...
                >
                  <div className="space-y-1">
                    <h3 className="text-[15px] font-black text-foreground">Alerts & Notifications</h3>
                  ...
                  <div className="flex items-center justify-between p-4 bg-card/30 border border-border rounded-2xl">
                    <div>
                      <h4 className="text-xs font-bold text-foreground">Weekly Entertainment Digest</h4>
                    ...
                    <button
                      ...
                      className={`w-10 h-6 rounded-full p-1 transition-colors ${emailDigest ? 'bg-accent' : 'bg-muted/20'}`}
                    >
                  ...
                  <div className="flex items-center justify-between p-4 bg-card/30 border border-border rounded-2xl">
                    <div>
                      <h4 className="text-xs font-bold text-foreground">Watchlist Release Notifications</h4>
                    ...
                    <button
                      ...
                      aria-pressed={releaseAlerts}
                      aria-label="Toggle release alerts"
                      className={`w-10 h-6 rounded-full p-1 transition-colors ${releaseAlerts ? 'bg-accent' : 'bg-muted/20'}`}
                    >
                  ...
                  <div className="flex items-center justify-between p-4 bg-card/30 border border-border rounded-2xl">
                    <div>
                      <h4 className="text-xs font-bold text-foreground">Recommendations & Newsletters</h4>
                    ...
                    <button
                      ...
                      className={`w-10 h-6 rounded-full p-1 transition-colors ${marketingEmails ? 'bg-accent' : 'bg-muted/20'}`}
                    >
              ...
              {activeTab === 'account' && (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-border space-y-3">
                    <h4 className="text-xs font-bold text-muted-foreground">Session Actions</h4>
                  ...
                  {/* Danger Zone */}
                  <div className="pt-4 border-t border-rose-500/10 space-y-3">
                    <div className="flex items-center gap-2 text-xs font-bold text-rose-400">
                      <AlertTriangle className="w-3.5 h-3.5" />
                      <span className="uppercase tracking-widest">Danger Zone</span>
                    </div>
                    <p className="text-[10.5px] text-muted-foreground leading-relaxed">
                      These actions are irreversible. Clearing local data will reset your cached preferences. Account deletion must be managed through Clerk.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button
