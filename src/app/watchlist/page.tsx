...
export default function WatchlistPage() {
  ...
  return (
    <div className="min-h-screen bg-[#07111F] pt-24 pb-28">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/[0.05]">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <h1 className="text-[22px] font-bold text-white tracking-tight">My Watchlist</h1>
            </div>
            <p className="text-[12.5px] text-muted">Manage your entertainment queues, track stats, and sort availability.</p>
          </div>
          {stats && (
            <div className="flex flex-wrap gap-2.5">
              <div className="glass-card px-3.5 py-2 rounded-xl text-center min-w-[70px]">
                <p className="text-[14px] font-bold text-white leading-none mb-1">{stats.count}</p>
                <p className="text-[9.5px] text-muted uppercase tracking-wider font-semibold">Saved</p>
              </div>
              <div className="glass-card px-3.5 py-2 rounded-xl text-center min-w-[70px]">
                <p className="text-[14px] font-bold text-white leading-none mb-1">{stats.hours}h</p>
                <p className="text-[9.5px] text-muted uppercase tracking-wider font-semibold">Runtime</p>
              </div>
              <div className="glass-card px-3.5 py-2 rounded-xl text-center min-w-[70px]">
                <p className="text-[14px] font-bold text-white leading-none mb-1">❤️ {favorites.length}</p>
                <p className="text-[9.5px] text-muted uppercase tracking-wider font-semibold">Favorites</p>
              </div>
            </div>
          )}
        </div>
      ...
      {stats && (
        <div className="max-w-[1280px] mx-auto px-6 lg:px-10 mb-8 grid grid-cols-1 md:grid-cols-[1.5fr_1fr] gap-4">
          <div className="glass-card p-5 rounded-2xl flex flex-col justify-between gap-4">
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-[10.5px] font-extrabold text-white/40 uppercase tracking-widest">
                <PieChart className="w-3.5 h-3.5" />
                Collection Insights
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-muted/50 uppercase tracking-wider">Favorite Provider</p>
                  <p className="text-[13px] font-semibold text-white/90 line-clamp-1">{stats.providers}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-muted/50 uppercase tracking-wider">Top Genres</p>
                  <p className="text-[13px] font-semibold text-white/90 line-clamp-1 capitalize">{stats.genres}</p>
                </div>
              </div>
            </div>
          ...
          <div className="glass-card p-5 rounded-2xl flex flex-col justify-center gap-3">
            <div className="text-center sm:text-left space-y-1">
              <h4 className="text-[13.5px] font-bold text-white">Need AI Recommendations?</h4>
              <p className="text-[11.5px] text-muted/60 leading-normal">
                Let's run a discovery pass based on your saved mood logs.
              </p>
            </div>
            <Link href="/discover" className="w-full">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full btn-primary py-2.5 text-[12px] uppercase tracking-wider font-black"
              >
                Scan Taste DNA
              </motion.button>
            </Link>
      ...
      <div className="max-w-[1280px] mx-auto px-6 lg:px-10 mb-6">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 py-2">
            ...
            {STATUS_TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-lg text-[12.5px] font-bold tracking-wide transition-all flex items-center gap-1.5 ${
                    active
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto justify-end">
            <button
              ...
              className={`h-9 px-3.5 rounded-xl border flex items-center gap-1.5 text-[12.5px] font-bold transition-all ${
        ...
        <AnimatePresence>
          {filtersOpen && (
            ...
            >
              <div className="glass-card p-5 rounded-2xl mt-3 grid grid-cols-1 sm:grid-cols-3 gap-6">
                {/* Provider Filter */}
                <div className="space-y-2">
                  <p className="text-[10px] font-bold text-muted/50 uppercase tracking-wider">Filter by Provider</p>
                  <select
                    value={providerFilter}
                    onChange={(e) => setProviderFilter(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 text-white rounded-xl text-[12.5px] p-2 focus:outline-none"
                  >
                ...
                {/* Availability Filter */}
                <div className="space-y-2">
                  <p className="text-[10px] font-bold text-muted/50 uppercase tracking-wider">Availability</p>
                  <div className="flex flex-wrap gap-1.5">
                    {AVAILABILITY_FILTERS.map(f => (
                      <button
                        key={f.id}
                        onClick={() => setAvailabilityFilter(f.id as any)}
                        className={`px-3 py-1.5 rounded-lg text-[11.5px] font-semibold transition-all border ${
                ...
                {/* Sorting */}
                <div className="space-y-2">
                  <p className="text-[10px] font-bold text-muted/50 uppercase tracking-wider">Sort List</p>
                  <div className="flex flex-wrap gap-1.5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 gap-y-8">
            {sorted.map((movie, i) => (
              <div key={movie.id} className="relative group">
                ...
                <div className="absolute bottom-16 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity bg-black/85 backdrop-blur-md rounded-lg p-1 border border-white/10 flex flex-col gap-1">
                  {STATUS_TABS.map(tab => (
                    <button
                      key={tab.id}
      ...
      <AnimatePresence>
        {selectedIds.length > 0 && (
          <motion.div
            ...
            className="fixed bottom-6 left-1/2 z-50 bg-[#07111F]/90 border border-white/[0.08] backdrop-blur-md px-5 py-3 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex items-center gap-4"
          >
...
const EmptyWatchlist: React.FC<{ activeTab: WatchlistStatus }> = ({ activeTab }) => (
  ...
  >
    ...
    <p className="text-[12.5px] text-muted/50 max-w-xs mb-6">
