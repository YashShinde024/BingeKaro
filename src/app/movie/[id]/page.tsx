        ...
        >
          ...
          >
            <motion.div
              ...
              className="w-44 lg:w-full rounded-card overflow-hidden border border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.85)] bg-[#121212]"
            >
              <motion.img
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
            ...
            <motion.div variants={fadeUp} className="space-y-2">
              {movie.tagline && (
                <p className="text-[12.5px] text-accent-light/70 font-semibold tracking-wide italic">"{movie.tagline}"</p>
              )}
              <motion.h1 
                layoutId={`title-${movie.id}`}
                className="text-3xl sm:text-4.5xl font-black text-white tracking-tight leading-tight"
              >
                {movie.title}
              </motion.h1>
            </motion.div>
            ...
            {/* Meta */}
            <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-2.5 text-white/60 text-[13px] font-medium">
              <span>{movie.year}</span>
              <span className="w-1 h-1 bg-white/10 rounded-full" />
              <div className="flex items-center gap-1 text-white font-semibold">
                <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                {movie.rating.toFixed(1)}
                <span className="text-[11px] text-white/40 font-normal">({(movie.voteCount / 1000).toFixed(0)}K)</span>
              </div>
              <span className="w-1 h-1 bg-white/10 rounded-full" />
              ...
              {movie.isFree ? (
                <>
                  <span className="w-1 h-1 bg-white/10 rounded-full" />
                  <span className="text-[9.5px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-md uppercase">
                    FREE TO WATCH
                  </span>
                </>
              ) : (
                <>
                  <span className="w-1 h-1 bg-white/10 rounded-full" />
                  <span className="text-[9.5px] font-bold text-violet-400 bg-violet-500/10 border border-violet-500/20 px-2 py-0.5 rounded-md uppercase">
                    PREMIUM TIER
                  </span>
                </>
              )}
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-1.5">
              {movie.genres.map(g => (
                <span key={g} className="px-3 py-1 rounded-full bg-white/[0.04] border border-white/[0.08] text-[11.5px] text-white/80 font-medium capitalize">
                  {g}
                </span>
              ))}
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="space-y-2.5">
              <p className="text-[10px] font-bold text-muted/40 uppercase tracking-widest">Platform Availability</p>
              ...
              >
                {activeTab === 'overview' && (
                  <div className="max-w-2xl space-y-4">
                    ...
                    <p className="text-[14.5px] text-white/75 leading-relaxed font-medium">{movie.overview}</p>
                  </div>
                )}
                    ...
                    {movie.cast && movie.cast.length > 0 ? movie.cast.map(member => (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-muted/60 truncate mt-0.5">{member.character}</p>
                        </div>
                      </motion.div>
                    )) : (
                ...
                {activeTab === 'similar' && (
                  <div className="flex gap-4 scroll-row pb-3 -mx-6 px-6 overflow-x-auto">
                  ...
                  <div className="max-w-2xl">
                    <div className="rounded-card p-5 border border-accent/20 bg-accent/[0.02] shadow-[0_4px_30px_rgba(139,92,246,0.05)] flex items-start gap-4">
                      ...
                      <div className="space-y-2">
                        <p className="text-[10px] font-bold text-accent-light uppercase tracking-widest">
        ...
        {showShare && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-[15px] font-bold text-white mb-1.5">Share Movie</h3>
              <p className="text-[12.5px] text-muted/60 mb-5">Share this screen with friends or other watch parties.</p>
