...
import { X, Mail, ArrowRight, ShieldCheck, Film, Eye, EyeOff, Loader2, User } from 'lucide-react';
...
export const AuthModal: React.FC = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        setError('Verification required. Please sign in via the full sign-in page.');
      }
    } catch (err: any) {
  ...
  const handleSocialAuth = async () => {
    const activeSdk = mode === 'signin' ? signIn : signUp;
    if (!activeSdk) return;
    try {
      await activeSdk.authenticateWithRedirect({
        strategy: 'oauth_google',
        redirectUrl: '/sso-callback',
        redirectUrlComplete: mode === 'signin' ? '/profile' : '/onboarding',
      });
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Bottom Navigation */}
        <div className="flex flex-col items-center gap-2 pt-2 text-[12px] text-muted-foreground border-t border-white/[0.04]">
          {mode === 'signin' ? (
            <>
              <button 
                onClick={() => { closeLoginModal(); router.push('/forgot-password'); }}
